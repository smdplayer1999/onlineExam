    ...
    override fun onAttachedToWindow() {
        ...
        if (!isInEditMode) {
            ...
            for (i in 0 until maxLength) {
                ...
                View.inflate(context, R.layout.item_code_edit_text, findViewById(R.id.llCodeWrapper))
            }
            ...
            for (i in 0 until llCodeWrapper.childCount) {
                ...
                val itemEdit= itemContainer.findViewById<TextView>(R.id.tvCode)
                itemEdit.setOnClickListener {

                editCodeReal.apply {
                    didClickButton(it,context)
                    showKeyboard()
                    focusOnLastLetter()
                }}
            }
    ...
    private fun renderCode() {
        for (i in 0 until llCodeWrapper.childCount) {
            ...
            if (editable.length > i){
                itemEdit.setBackgroundColor(Color.BLUE)
            }else{
                itemEdit.setBackgroundColor(Color.LTGRAY)
            }
                itemEdit.text = if (editable.length > i){
                    (if (maskTheCode) codeMaskChar
                    else editable[i]).toString()
                }
